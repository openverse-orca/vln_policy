...
INITIAL_ARM_YAWS = np.deg2rad([-90, -75, -60,-45, -30,-15, 0, 15, 30, 45, 60, 75, 90, 0]).tolist()
# INITIAL_ARM_YAWS = np.zeros(100).tolist()  # type: ignore
from vlfm.utils.display_utils import display_img
...
class RealityMixin:
    ...
    def __init__(self: Union["RealityMixin", ITMPolicyV2], *args: Any, **kwargs: Any) -> None:
        ...
        # # 检查 ~/.cache/torch/hub/checkpoints 下是否存在 ZoeD_M12_NK.pt
        checkpoint_path = os.path.expanduser("~/.cache/torch/hub/isl-org_ZoeDepth_main")
        if os.path.exists(checkpoint_path):
            self._depth_model = torch.hub.load(
                checkpoint_path, "ZoeD_NK", config_mode="eval", pretrained=True ,source='local'
            ).to("cuda" if torch.cuda.is_available() else "cpu")
            # self._depth_model.load_state_dict(torch.load(checkpoint_path))
        else:
            self._depth_model = torch.hub.load(
    ...
    def act(
        ...
        if self._done_initializing:
            action_dict = {
                ...
                "linear": action[0][1].item() if action.size(1) > 1 else 0,
                "arm_yaw": -1,
    ...
    def _cache_observations(self: Union["RealityMixin", ITMPolicyV2], observations: Dict[str, Any]) -> None:
        ...
        #     print("range--position", observations["robot_xy"],"---tf_pos：", tf[:2, 3])
