...
from  vlfm.utils.display_utils import display_img, display_grayscale
...
class VLFMTrainer(PPOTrainer):
    ...
    def _eval_checkpoint(
        ...
        while len(stats_episodes) < (number_of_eval_episodes * evals_per_ep) and self.envs.num_envs > 0:
            ...
            #     display_img(obs["rgb"], 100, "rgb")
            #     display_grayscale(obs["depth"], 100)
            #     print(obs["frontier_sensor"])
                # base_explorer
                # compass
                # depth
                # frontier_sensor
            ...
            # 打印变换后的 rgb 和 depth 的 shape
            # if "rgb" in batch:
            #     print(f"Transformed RGB shape: {batch['rgb'].shape}")
            # if "depth" in batch:
            #     print(f"Transformed Depth shape: {batch['depth'].shape}")
            ...
            for i in range(n_envs):
                ...
                # episode 结束
                # 累加奖励到 episode_stats ，提取信息（如success标记）。
                # 通过 log_episode_stats 记录详细日志（如失败原因）。
